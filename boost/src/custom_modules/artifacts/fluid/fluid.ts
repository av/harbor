...
import { rnd, rndInt, normalizeColor, wrap, lmap } from "./utils";
import { compileShader, getWebGLContext, Material, Program } from "./webgl";
...
export class FluidSim {
  ...
  clusterSplat(config = {}) {
    const wiggle = config?.wiggle ?? 0.05;
    const position = {
      x: config?.position?.x ?? rnd(wiggle, 1.0 - wiggle),
      y: config?.position?.y ?? rnd(wiggle, 1.0 - wiggle),
    };

    const amount = config?.amount ?? rndInt(3, 8);
    for (let i = 0; i < amount; i++) {
      const color = {
        r: 0.12,
        g: 0.12,
      ...
      const x = position.x + rnd(-wiggle, wiggle);
      const y = position.y + rnd(-wiggle, wiggle);
      const dx = 200 * rnd(-1.0, 1.0);
      const dy = 200 * rnd(-1.0, 1.0);
      fluidSim.splat({ x, y, dx, dy, color });
    }
  }

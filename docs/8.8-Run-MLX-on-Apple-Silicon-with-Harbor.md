...
```bash
harbor config set mlx.model mlx-qwen3-8b
harbor config set mlx.hf.path mlx-community/Qwen3-8B-4bit
```

Restart the host runner and proxy:
...
The next `harbor up mlx` (or any stack that includes mlx) will launch the new model.
...
- If you manage mlx-lm manually, disable host management and point at your endpoint

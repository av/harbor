...
> Handle: `flowise`<br/>
> URL: [http://localhost:34281](http://localhost:34281)
...
[Official environment variables reference](https://github.com/FlowiseAI/Flowise/blob/main/CONTRIBUTING.md#-env-variables). To set them, see Harbor's [environment configuration guide](./1.-Harbor-User-Guide.md#environment-variables).
...
> [!NOTE]
> The version is pinned to `3.1.3` — `3.1.4` and current `latest` crash at startup (upstream session-store bug: a filename string is passed where the vendored `connect-sqlite3` expects an open database handle).

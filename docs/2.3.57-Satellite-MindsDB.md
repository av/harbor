...
MindsDB is an AI platform that enables querying AI models using SQL. Harbor integrates MindsDB with local LLM backends (Ollama, llama.cpp, vLLM, Docker Model Runner, MLX, oMLX) so you can build AI-powered applications without external API dependencies.
...
# Persistent storage
HARBOR_MINDSDB_WORKSPACE           ./services/mindsdb/data
...
HARBOR_MINDSDB_OLLAMA_MODEL        qwen3.5:4b

# Default llama.cpp model for LLM() function
HARBOR_MINDSDB_LLAMACPP_MODEL      unsloth/Qwen3.5-4B-GGUF:Q4_K_M
```
...
| llama.cpp | `http://llamacpp:8080/v1` | `HARBOR_MINDSDB_LLAMACPP_MODEL` | `sk-llamacpp` |
| vLLM | `http://vllm:8000/v1` | `HARBOR_VLLM_MODEL` | `sk-vllm` |
...
> **Note**: The `LLM()` function configuration is separate from the "Default MindsDB Configuration" UI settings. UI settings (Settings → Models) are for agents/knowledge bases and persist in `./services/mindsdb/data`.

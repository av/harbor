...
Harbor can run an Ollama Open WebUI Docker Compose setup as a local chat stack, then add SearXNG when you want Open WebUI SearXNG web search for local web RAG workflows.

This guide focuses on the practical path: start Harbor's Ollama and Open WebUI services, add SearXNG, then use Open WebUI's web search features without maintaining a separate Compose project by hand.
...
Open WebUI is one of Harbor's default services. This guide uses Ollama as the local backend, so start both explicitly:
...
![WebUI with SearXNG Web RAG](seaxng-webrag.png)
*Example of Open WebUI using SearXNG for web search to answer a current-events query.*
...
If you are still choosing the overall stack shape, start with [Local LLM Stack with Docker Compose](./8.1-Local-LLM-Stack-with-Docker-Compose.md), then return here when you want local AI web search attached to your Harbor chat setup.

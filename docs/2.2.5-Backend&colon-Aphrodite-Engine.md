...
> **NVIDIA-only.** The upstream `alpindale/aphrodite-openai` images are CUDA-only — this service requires an NVIDIA GPU with the NVIDIA Container Toolkit installed. It will not run on CPU-only or AMD/ROCm hosts.

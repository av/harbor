...
A local OpenAI compatible API is useful when you want the same model stack to serve browser frontends, agent tools, scripts, and development CLIs. Harbor gives you several local backend choices, including [Ollama](./2.2.1-Backend&colon-Ollama.md), [llama.cpp](./2.2.2-Backend&colon-llama.cpp.md), [vLLM](./2.2.3-Backend&colon-vLLM.md), [Docker Model Runner](./2.2.22-Backend-Docker-Model-Runner.md), [MLX](./2.2.23-Backend-MLX.md), and [oMLX](./2.2.24-Backend-oMLX.md), then wires compatible services to those backends through Docker Compose.
...
![Ollama Model Provider in Open WebUI](harbor-ollama-ls.png)
*Open WebUI model provider settings showing Ollama integration with available local models.*

[Ollama](./2.2.1-Backend&colon-Ollama.md) is a popular Harbor backend because it is convenient for most users. Start it alongside Open WebUI:

```bash
harbor up webui ollama
```

Use Ollama when you want simple model management, a familiar local runtime, and a broad integration path in Harbor. The Ollama service doc describes Harbor's Ollama OpenAI compatible API path, including how many Harbor consumers use the service's `/v1` endpoint and conventional local key.
...
Treat vLLM as a heavier backend than the Ollama path. The [vLLM service doc](./2.2.3-Backend&colon-vLLM.md) covers its local image build, Hugging Face cache, GPU overlays, ROCm path, and host IPC behavior; choose a model and quantization strategy that fit your GPU or accelerator capacity before making it part of your default stack.

### Docker Model Runner

[Docker Model Runner](./2.2.22-Backend-Docker-Model-Runner.md) is useful when you want Docker to manage the local model runtime while Harbor services consume a normal OpenAI-compatible endpoint.
...
For a longer walkthrough (model search, pulls, and multi-platform setup), see [Docker Model Runner with Harbor](./8.9-Docker-Model-Runner-with-Harbor.md).

### MLX

[MLX](./2.2.23-Backend-MLX.md) is useful when you specifically want Apple's MLX runtime on an Apple Silicon Mac. Harbor manages `mlx-lm` on the host and exposes it to the Compose network through a proxy container.
...
For HuggingFace discovery, pulls, and Apple Silicon stack examples, see [Run MLX on Apple Silicon with Harbor](./8.8-Run-MLX-on-Apple-Silicon-with-Harbor.md).

### oMLX

[oMLX](./2.2.24-Backend-oMLX.md) is useful when you want an Apple Silicon MLX backend with multi-model serving, continuous batching, a web admin dashboard, and paged SSD KV caching. Harbor manages `omlx serve` on the host and exposes it to the Compose network through a proxy container.
...
The default Harbor profile starts Open WebUI and llama.cpp. You can start another backend directly:

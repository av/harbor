...
`--workflow` starts Boost and SearXNG when required and routes the tool to `{preset}-{model}`
(e.g. `shipyard-qwen3.5:4b`). Cannot combine `--web` and `--workflow`. Mount your project
with `boost.workspace` and set `boost.workspace.root` to `/workspace` so workspace tools and
`autocheck` can read and verify cited file paths.

...
- Not included in built-in presets; add manually when scratch or workspace
  read-before-edit is required
- Ad hoc sandbox example: `agent-code=tools,sightline,final` with
  `HARBOR_BOOST_TOOLS` including `read_workspace_file;write_workspace_file;grep_workspace;list_workspace_files`
...
Harbor Boost ships five built-in workflow presets for agentic coding. Each
...
`sightline` and `diffscope` are not in built-in presets — add them via custom
`HARBOR_BOOST_WORKFLOWS` entries when scratch or workspace read-before-edit, or
file-scope enforcement, is required (see those module sections). Workspace writes
(`write_workspace_file`) are also opt-in via `HARBOR_BOOST_TOOLS`, not enabled by
default in `shipyard` or other presets.

...
The `shipyard` preset chains the agentic modules in ideation order: `keel`
anchors the task, `caveman` runs selective ideation research, `tools` registers
portable helpers, `ponytail` performs deeper implementation research when needed,
and `autocheck` audits coding deliverables before the final completion. Prep steps
use `defer_final` so the workflow can continue through the chain.

...
`list_workspace_files`, and opt-in `write_workspace_file`) for full sandbox use.
...
**Session:** anchor the task (*"add rate limiting to `api/routes/users.py` only"*) → agent
researches and edits → deliverable turns pass through `autocheck` before you see patches.
...
`write_workspace_file`) are also available. `write_workspace_file` is **not** in

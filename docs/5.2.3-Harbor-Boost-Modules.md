...
```bash
harbor config set HARBOR_BOOST_WORKSPACE_ROOT "$(pwd)"
harbor config update
...
(e.g. `shipyard-qwen3.5:4b`). Cannot combine `--web` and `--workflow`. Set
`HARBOR_BOOST_WORKSPACE_ROOT` so `autocheck` can verify cited file paths.

...
`caveman`, `ponytail`, and the `tools` module's `web_search` helper all require a
...
harbor boost modules add caveman   # or ponytail, or a workflow preset
...
| `--workflow <preset>` | Starts Boost; **auto-starts SearXNG** when the preset includes web research modules |

Presets that auto-start SearXNG: `research-quick`, `research-deep`, `agent-research`,
and `shipyard` (all use `caveman` and/or `ponytail`). Presets without web research
(`code-check`, `scope-guard`, `agent-code`) do not start SearXNG.

```bash
# Same SearXNG wiring as --web, but uses the research-quick preset (tools + caveman)
...
> **Requires web search:** Configure [SearXNG or Tavily](#web-search-requirement) like
> the `--web` launch path, or use `harbor launch --workflow` for presets that
> auto-start SearXNG.
...
is throttled to every N user turns (`HARBOR_BOOST_KEEL_ANCHOR_EVERY`, default `2`).
Simple drift heuristics flag scope-expansion phrases. A landing checklist with
...
harbor config set HARBOR_BOOST_KEEL_ANCHOR_EVERY 2
```
...
- Pair with `HARBOR_BOOST_TOOLS` including
  `read_workspace_file;write_workspace_file;grep_workspace;list_workspace_files` when
...
Harbor Boost ships six built-in workflow presets for agentic coding. Each
preset registers portable `tools` as a setup step (`final: false`) where needed,
runs research or verification modules, then streams the downstream completion.

### Workspace bind mount

Boost runs in Docker. `HARBOR_BOOST_WORKSPACE_ROOT` must point at a path **inside**
the container, not a host path like `$(pwd)`. Harbor bind-mounts a host directory
to `/workspace` in `compose.boost.yml`:

| Config key | Env var | Container path |
|------------|---------|----------------|
| `boost.workspace` | `HARBOR_BOOST_WORKSPACE` | Host folder mounted at `/workspace` |
| `boost.workspace.root` | `HARBOR_BOOST_WORKSPACE_ROOT` | Jail root for workspace tools |

When `HARBOR_BOOST_WORKSPACE` is empty, Compose falls back to
`./services/boost/workspace` (a Harbor-managed sandbox). Override with your
project root for real repo access:

```bash
# Mount the current project and enable workspace tools
...
Alternative without changing `HARBOR_BOOST_WORKSPACE`: add a custom mount and set
the in-container root to match:

```bash
harbor volumes add boost "$(pwd):/workspace"
harbor config set boost.workspace.root /workspace
harbor restart boost
```

`write_workspace_file` is opt-in — add it to `HARBOR_BOOST_TOOLS` only for
sandboxes that should allow edits. Pair with `sightline` in `agent-code` or
`shipyard` presets.

### Choosing a preset

**Which workflow?** (agent quick-pick)

```mermaid
flowchart TD
    root["Which workflow?"]
    root --> q1{"Quick lookup?"}
    q1 -->|yes| rq["research-quick"]
    q1 -->|no| q2{"Deep migration?"}
    q2 -->|yes| rd["research-deep"]
    q2 -->|no| q3{"Bugfix + scope?"}
    q3 -->|yes| sg["scope-guard"]
    q3 -->|no| q4{"Sandbox coding?"}
    q4 -->|yes| ac["agent-code"]
    q4 -->|no| sy["shipyard"]
```

| Task | Preset |
|------|--------|
| Quick lookup (docs, errors) | `research-quick` |
| Deep migration / API compare | `research-deep` |
| Bugfix with user-stated scope | `scope-guard` |
| Full sandbox coding | `agent-code` |
| Long multi-turn agent session | `shipyard` |

Enable via `harbor launch --workflow <preset>` or `@boost_workflow` in request metadata.

| Question | Prefer |
|----------|--------|
| Quick lookup before answering? | `research-quick` (`caveman`) |
| Migration, API, or version comparison? | `research-deep` (`ponytail`) |
| Audit code changes before delivery? | `code-check` (`autocheck`, opt-in) |
| Focused bugfix with user-stated file scope? | `scope-guard` (`diffscope`, `autocheck`) |
| Tool-enabled agent session with fast research? | `agent-research` (`caveman`) |
| Sandbox coding with workspace tools and hardening? | `agent-code` (`sightline`, `diffscope`, `autocheck`) |
| Full multi-turn coding pipeline? | `shipyard` |

**`caveman` vs `ponytail`:** Both skip acknowledgment and implementation-only turns.
Use `caveman` for fast smash-and-grab research (lower latency, 2 searches).
Use `ponytail` for research-heavy questions that benefit from a second targeted
search hop (higher budgets, structured brief with uncertainties).

**`autocheck` is opt-in:** Add the module or pick `code-check` / `shipyard`.
It only runs on coding deliverable turns (≥2 deliverable signals); explanations
and short acks pass through unchanged.
...
| `scope-guard` | `tools`, `diffscope`, `autocheck`, `final` | Focused bugfix with user-stated file scope and deliverable audit |
| `agent-research` | `tools`, `caveman`, `final` | Tool-enabled research during agentic coding sessions |
| `agent-code` | `tools`, `sightline`, `diffscope`, `autocheck`, `final` | Sandbox coding with workspace tools, read-before-edit, scope checks, and deliverable audit |
| `shipyard` | `keel`, `caveman`, `tools`, `ponytail`, `autocheck`, `final` | Full agentic coding pipeline from grounding through audit |

The `shipyard` preset chains modules in ideation order: `keel` anchors the task,
`caveman` runs selective ideation research, `tools` registers portable helpers,
`ponytail` performs deeper implementation research when needed, and `autocheck`
audits coding deliverables before the final completion. Prep steps use `defer_final`
so the workflow continues through the chain.

#### Shipyard turn types

Each user turn is classified so modules can skip expensive work when it would not
help:

| Turn type | Example | `keel` | `caveman` | `ponytail` | `autocheck` |
|-----------|---------|--------|-----------|------------|-------------|
| **Research-only** | *"What is the Stripe checkout API response format?"* | pass-through | runs | runs | pass-through (`research_only`) |
| **Implementation** | *"Implement auth migration in `auth.py` using latest docs"* | extracts/anchors brief | **skipped** when brief is implementation-focused | runs when research signals present | runs on deliverable turns (≥2 signals) |
| **Ack / continue** | *"thanks"*, *"continue"* | pass-through | skipped | skipped | pass-through |
| **Completion** | *"ship it"* after prior coding work | landing checklist | skipped | skipped | triggered via completion signal |

**`defer_final`:** `keel`, `caveman`, `ponytail`, and `autocheck` defer streaming
the downstream completion when configured in the preset. Only the explicit `final`
step (or a standalone module request) streams the answer. `tools` uses `final: false`
as a setup step in the same spirit.

**Caveman vs ponytail in shipyard:** After `keel` anchors an implementation brief,
`caveman` skips so ideation research does not duplicate `ponytail`'s deeper
implementation research hop. Pure research questions (no code-change intent) still
use `caveman` for fast lookups.

The `scope-guard` preset is a lightweight alternative for focused bugfixes where
the user states file scope (`only X`, `don't touch Y`). It chains `diffscope` for
scope enforcement and `autocheck` for deliverable audit without `sightline`, `keel`,
or research modules.

The `agent-code` preset chains sandbox hardening after `tools`: `sightline` guards
read-before-edit, `diffscope` enforces file-scope constraints, and `autocheck`
audits coding deliverables. Set `HARBOR_BOOST_WORKSPACE_ROOT` and add workspace
tools to `HARBOR_BOOST_TOOLS` (`read_workspace_file`, `grep_workspace`,
`list_workspace_files`, `git_diff_workspace`, and opt-in `write_workspace_file`)
for full sandbox use.
Workspace writes are not enabled by default in `agent-code`, `shipyard`, or other
presets. For custom chains (e.g. `scope-check` with `keel`), override via
...
Three common agentic-coding flows using built-in presets and [`harbor launch`](./3.-Harbor-CLI-Reference.md#harbor-launch-launch-options---service-servicetool-args).

#### 1. Quick API lookup (`research-quick` + `caveman`)

Fast doc or error lookup before the agent answers. `caveman` runs 1–2 budget-capped
searches and injects a compact `<research_brief>`; implementation-only turns skip research.

```bash
# Starts boost + searxng; routes codex to research-quick-<model>
harbor launch --workflow research-quick --backend ollama --model qwen3.5:4b codex \
  -p "What header does the GitHub REST API require for fine-grained PAT auth?"

harbor launch --workflow research-quick --backend ollama --model qwen3.5:4b opencode
```

**Prompt:** *"Look up the Stripe Checkout Session create endpoint — which fields set success/cancel URLs?"*

#### 2. Migration research (`research-deep` + `ponytail`)

Migrations, version comparisons, and breaking-change questions. `ponytail` plans queries,
reads top pages, runs a targeted second search hop, then synthesizes uncertainties.

```bash
harbor launch --workflow research-deep --backend ollama --model qwen3.5:4b codex \
  -p "Migrate this Express 4 app to Fastify 5 — list breaking changes we must handle."

# Print Boost routing without starting the tool
harbor launch --workflow research-deep --backend ollama --model qwen3.5:4b --config codex
# backend=boost  model=research-deep-qwen3.5:4b
```

**Prompt:** *"We're on SQLAlchemy 1.4 — what changes for 2.0 declarative models and session usage?"*

#### 3. Full coding session (`shipyard`)

Full pipeline: `keel` grounding → selective `caveman` ideation → `tools` → `ponytail`
implementation research → `autocheck` deliverable audit → final completion.
...
**Multi-turn session** (same `shipyard` workflow on every turn):

| Turn | You say | What runs |
|------|---------|-----------|
| **1 — research** | *"What fields does Stripe Checkout Session return for `customer_email`?"* | `keel` pass-through · **`caveman`** (1–2 fast searches → `<research_brief>`) · `ponytail` may add a second hop · `autocheck` skipped (`research_only`) |
| **2 — implementation** | *"Migrate checkout auth in `services/boost/src/auth.py` using latest Stripe API docs"* | `keel` extracts/anchors brief · **`caveman` skipped** (implementation brief already anchored) · **`ponytail` may run** (docs/migration signals) · `tools` registered for the completion |
| **3 — deliverable** | *"Apply the migration patch to `auth.py` and show the diff"* | `keel` anchors scope · research modules skipped · **`autocheck`** drafts → audits cited paths → optional revise → `final` streams answer |

Turn 1 is a pure lookup: research modules gather facts; `autocheck` stays off so latency stays low.
Turn 2 shifts to scoped implementation — `keel` stores objective and `in_scope_paths`, so `caveman`
does not repeat ideation research while `ponytail` can still deep-read breaking-change docs.
Turn 3 carries two deliverable signals (coding action + file path), so `autocheck` verifies
`auth.py` against `/workspace` before you see the patch.

[`harbor launch --workflow`](./3.-Harbor-CLI-Reference.md#harbor-launch-launch-options---service-servicetool-args)
starts Boost and auto-starts SearXNG for research presets (`research-quick`,
`research-deep`, `agent-research`, `shipyard`), then routes the tool to
`{preset}-{model}` (e.g. `shipyard-qwen3.5:4b`). See [Web search requirement](#web-search-requirement)
for manual SearXNG/Tavily setup when not using `harbor launch`. Cannot combine `--web`
and `--workflow`. Mount your project with `boost.workspace` and set `boost.workspace.root`
to `/workspace` so workspace tools and `autocheck` can read and verify cited file paths.
...
`write_workspace_file` is **not** in
the default tool list — add
it to `HARBOR_BOOST_TOOLS` for agentic coding sandboxes that should allow edits.
Pair with `sightline` for read-before-edit guarding. Web search uses

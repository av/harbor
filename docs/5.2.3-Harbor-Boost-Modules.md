...
Agentic Boost modules and the [`tools`](#tools) module's workspace file helpers require a host directory bind-mounted into the container and an in-container jail root. Without both, workspace file tools are disabled and modules like [`autocheck`](#autocheck), [`diffscope`](#diffscope), [`sightline`](#sightline), and [`keel`](#keel) cannot verify cited file paths against your repo.
...
# Enable workspace tools for agent-code sandboxes
...
Presets that benefit from a mounted project:

| Preset | Modules | Workspace use |
|--------|---------|---------------|
| `agent-code` | `tools`, `sightline`, `diffscope`, `autocheck` | Full sandbox with read-before-edit guards |
| `shipyard` | `tools`, `caveman`, `ponytail`, `keel`, `autocheck` | Keel compares git diff against acceptance criteria |
| `code-check` | `tools`, `autocheck` | Deliverable-turn audit with path grounding |
| `scope-guard` | `tools`, `diffscope`, `autocheck` | Scoped bugfix with path verification |

[`harbor launch`](./3.-Harbor-CLI-Reference.md#harbor-launch-launch-options---service-servicetool-args) with `--sandbox workspace-write` wires workspace tools for `agent-code`.

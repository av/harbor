...
docker pull ghcr.io/all-hands-ai/runtime:latest-nikolaik
...
OpenHands stores its state (settings, conversations) in `services/openhands/state`, bind-mounted to `/.openhands` in the container. An `openhands-init` sidecar chowns this directory to your host user before OpenHands starts — the image's entrypoint remaps its app user to `SANDBOX_USER_ID`, and a root-owned state directory would otherwise crash the first boot. Both the app and runtime images come from `ghcr.io/all-hands-ai` (`openhands` and `runtime:<version>-nikolaik`), pinned by `HARBOR_OPENHANDS_VERSION` (default `latest`).
...
- [`harbor url -i ollama`](./3.-Harbor-CLI-Reference.md#harbor-url-service) - get the internal URL of the `ollama` service

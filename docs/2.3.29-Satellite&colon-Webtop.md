...
Paired with [`harbor profiles`](./3.-Harbor-CLI-Reference.md#harbor-profile), you can use that to "switch" between different home directories for different projects.
...
In addition to that, all of the baseline configuration options from original service are supported. One quick way to adjust the environment is via [`harbor env`](./3.-Harbor-CLI-Reference.md#harbor-env):
...
harbor env webtop set TITLE "AI Webtop"
```
...
See more info in [`harbor url` reference](./3.-Harbor-CLI-Reference.md#harbor-url).

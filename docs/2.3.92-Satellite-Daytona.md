...
Handled automatically: a `daytona-dex-init` sidecar pre-creates and chowns `services/daytona/data/dex/` before Dex starts, and the Dex container runs as the host user (`HARBOR_USER_ID`, fallback 1001) so its SQLite state stays host-owned and deletable without `sudo`. If you still see "Permission denied" from an install predating this (dir owned by another UID), reset ownership once:

```bash
docker run --rm -v ./services/daytona/data:/data alpine:3.20 chown -R "$(id -u):$(id -g)" /data/dex
harbor up daytona
```

...
registered as a boost backend with MiniMax-M2.7 (default),
MiniMax-M2.7-highspeed, and previous MiniMax models.

```bash
# Set via Harbor CLI
harbor config set minimax.api_key sk-your-minimax-key

# MiniMax models will then appear in boost's model list
```
...
> **Type**: `StrList`<br/>
> **Default**: `klmbr;rcn;g1;mcts;eli5;supersummer`<br/>
...
A semicolon-separated list of API keys to use for the boost API. Counterpart to `HARBOR_BOOST_API_KEY`.

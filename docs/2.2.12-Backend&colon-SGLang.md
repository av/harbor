...
> **NVIDIA-only.** The upstream `lmsysorg/sglang` images are CUDA-only — this service requires an NVIDIA GPU with the NVIDIA Container Toolkit installed. It will not run on CPU-only or AMD/ROCm hosts.
...
harbor sglang model Qwen/Qwen3.5-4B

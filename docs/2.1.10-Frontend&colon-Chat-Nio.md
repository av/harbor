...
# App-level secret (JWT signing). Chat Nio requires at least
# 32 bytes; Harbor injects this value into the runtime config at startup.
# Change it for any non-local deployment.
HARBOR_CHATNIO_SECRET          harbor-chatnio-secret-change-me-32chars
```

...
# only set it explicitly when detection fails
...
|---|---|---|
| `HARBOR_MISTRALRS_VERSION` | `0.6.0` | mistral.rs image version tag. Note: the `cpu-` variants of 0.7.0–0.9.0 are broken upstream (binary built against a newer glibc than the image base); 0.6.0 is the newest working CPU image |

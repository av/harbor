...
- OpenClaw requires an OpenAI-compatible backend. Start with one of: `ollama`, `llamacpp`, `vllm`, `tabbyapi`, `mistralrs`, `sglang`, `lmdeploy`, or [`unsloth-studio`](./2.3.83-Satellite-Unsloth-Studio.md).
- Harbor automatically configures OpenClaw to use whichever backend is running.
- For [`unsloth-studio`](./2.3.83-Satellite-Unsloth-Studio.md): OpenClaw waits on the `unsloth-studio-bootstrap` sidecar and reads the freshly-minted API key at runtime — `harbor up unsloth-studio openclaw` works on first launch.

...
The container runs as your host user (via `HARBOR_USER_ID`, plus supplementary gid 0 for the group-writable `/app` in the upstream image); a `pipelines-init` sidecar chowns `./services/pipelines/persistent` before startup, so pipeline files stay owned by your host user.

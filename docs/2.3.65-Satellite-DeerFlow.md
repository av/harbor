...
- Harbor builds both images from the upstream repository at the ref in `HARBOR_DEERFLOW_GIT_REF` (`harbor config get deerflow.git_ref`). It is pinned to the last v1.x commit — upstream's v2 rewrite restructured the repository (`backend/`, `frontend/`) and is not yet supported
...
# Note: Must use a model with proper tool calling support (e.g., qwen3.5:4b, mistral)
...
- Base fragment: `services/deerflow/configs/deerflow.config.yaml`
...
- `services/deerflow/data/` - research reports, cache, and generated content
- `services/deerflow/configs/` - Harbor-managed configuration fragments
...
I've tested that `qwen3.5:4b` and stronger tool-capable models work well.

Change the model:

```bash
harbor config set deerflow.model "llama3.1:8b"

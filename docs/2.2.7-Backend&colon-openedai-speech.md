...
# Spin up Harbor with the TTS instance
harbor up tts
```

Upon the first start, service will initialise its cache and download the necessary models. You can find both in the `services/tts` folder in the Harbor workspace. The container runs as your host user (an init sidecar fixes up ownership of pre-existing files), so downloaded voices and configs stay owned by you — no `sudo` needed to manage or delete them.
...
You can map your [Piper voices](https://rhasspy.github.io/piper-samples/) via the [`services/tts/config/voice_to_speaker.yaml`](../services/tts/config/voice_to_speaker.yaml) file.

...
The following options can be set via [`harbor config`](./3.-Harbor-CLI-Reference.md#harbor-config) or in `services/karakeep/override.env`:
...
# Data directory
HARBOR_KARAKEEP_WORKSPACE  # default: ./services/karakeep/data
```
...
You can configure the inference models in `services/karakeep/override.env`:
...
Harbor leaves both inference model defaults empty, so choose and set them explicitly before relying on AI tagging. For example:

```bash
harbor config set karakeep.inference_text_model qwen3.5:4b
...
- `./services/karakeep/data` - Bookmark data, assets, and database
- `./services/karakeep/data/meilisearch` - Search index data

#### File ownership

Karakeep and its Meilisearch sidecar run as your host user (via `HARBOR_USER_ID`), with s6-overlay operating rootless over an exec-mounted `/run` tmpfs. Files written into `./services/karakeep/data` stay owned by your host user, so the workspace can be managed without `sudo`.

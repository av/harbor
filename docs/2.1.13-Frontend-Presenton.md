...
# Or start with llama.cpp for local inference
harbor up presenton llamacpp --open
```

#### Usage

- When started together with `ollama`, Harbor connects Presenton to it automatically
- When started together with `llamacpp`, Harbor wires Presenton's `custom` provider to llama.cpp's OpenAI-compatible API; pick the served model via:

```bash
# Must be a model id llamacpp actually serves (see /v1/models);
# Ollama-style tags match nothing here
harbor config set presenton.llamacpp.model "LiquidAI/LFM2.5-8B-A1B-GGUF:Q8_0"
```
...
# Auth: Presenton requires an interactive login/password setup on first
# boot unless auth is disabled. Harbor disables it by default (local
# single-user tool); set to "false" to use Presenton's built-in auth.
HARBOR_PRESENTON_DISABLE_AUTH="true"
```
...
- Check that `./services/presenton/app_data` directory exists and is writable
- Verify volume mount in `harbor logs presenton`

#### See Also

- [Official Presenton Documentation](https://docs.presenton.ai)
- [Ollama Backend](./2.2.1-Backend&colon-Ollama.md)
- [Harbor Config Reference](./3.-Harbor-CLI-Reference.md#harbor-config)

#### File ownership

Presenton starts through a Harbor entrypoint wrapper that fixes ownership as root and then drops to your host user (via `setpriv`) — all processes, including the internal nginx, run non-root, so files in `./services/presenton/app_data` stay owned by your host user.

...
- `ollama` — `LLM_PROVIDER=ollama`, `OLLAMA_BASE_PATH` (and the parallel settings for the embedding engine) point at the Ollama container using the Harbor-internal URL. Harbor seeds `nomic-embed-text:latest` as the embedding model only if the persistent settings file has no selection; chat and embedding models can then be changed inside the AnythingLLM UI without being reset by the container environment.
...
- **Data persistence**: the directory `$(harbor home)/services/anythingllm/storage` on the host is mounted at `/app/server/storage` inside the container. Its `.env` file is also mounted at `/app/server/.env`, where AnythingLLM writes model choices and other instance settings. The init sidecar creates that file without replacing existing contents, assigns it to the app user, and restricts it to mode `0600`. Workspaces, uploaded files, and LanceDB vectors remain in the same storage directory. This file can contain API keys; do not commit or share it. (See the file-system permissions guidance under Starting.)
...
The integrations are entirely declarative through the compose overlays. If you restart only `anythingllm` by name, Harbor does not include a co-running backend in that restart's Compose selection. Run `harbor up anythingllm ollama` (or name your chosen backend) afterward to restore the integration without restarting the backend. Saved model choices remain in the settings file.

If you are upgrading from a Harbor version that did not mount `/app/server/.env`, copy settings from the *still-running old container* before recreating it, provided the host file does not already exist:

```bash
docker cp harbor.anythingllm:/app/server/.env "$(harbor home)/services/anythingllm/storage/.env"
chmod 600 "$(harbor home)/services/anythingllm/storage/.env"
```

After the old container has been removed, its unmounted `.env` cannot be recovered from Harbor's storage directory; re-enter those settings in AnythingLLM if needed.

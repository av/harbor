...
### `harbor launch <service> [args]`

Launches a Harbor service CLI with the currently running Harbor services included in the compose selection. This is useful for agent and tool CLIs that should reuse an already-running Harbor inference backend instead of starting or configuring a separate backend.

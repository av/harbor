...
- **mlx** — calls mlx-serve through `HARBOR_MLX_UPSTREAM_URL`.
...
With `--source dmr`, Harbor delegates to Docker Model Runner. With `--source mlx`, Harbor delegates to the mlx-serve model management API exposed by `HARBOR_MLX_UPSTREAM_URL`.
...
- **mlx** — delegates removal to the mlx-serve model management API.
...
| `mlx.upstream.url` | mlx-serve endpoint used for listing models |

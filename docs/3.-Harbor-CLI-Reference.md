...
### `harbor launch <service|tool> [args]`
...
For known Harbor service CLIs, `launch` dispatches to the same implementation as the direct command, such as `harbor mi`, `harbor promptfoo`, or `harbor plandex`. For other service handles it falls back to [`harbor run`](#harbor-run-service-command).

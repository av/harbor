...
Cross-source model management for Ollama, HuggingFace Hub, llama.cpp caches, Docker Model Runner, and MLX. By default, cache operations preserve the existing Ollama/HuggingFace/llama.cpp routing. Use `--source` when a model belongs to Docker Model Runner or MLX.

> Aliases: `harbor models list` = `harbor models ls`, `harbor models remove` = `harbor models rm`, `harbor models dmr pull ...` = `harbor models pull --source dmr ...`

#### `harbor models ls`

List locally available models across Ollama, HuggingFace Hub, the llama.cpp GGUF cache, Docker Model Runner, and MLX. Host-backed DMR and MLX are queried by default only when their Harbor proxy service is running; use `--source dmr` or `--source mlx` to query their host endpoints directly.
...
With `--source dmr`, Harbor delegates to Docker Model Runner. With `--source mlx`, Harbor delegates to `harbor mlx pull`, which downloads HuggingFace model repos into the local cache.
...
Manages [Docker Model Runner](./2.2.22-Backend-Docker-Model-Runner.md) — a host-native backend where Docker Desktop runs inference directly on the host (Metal on macOS, GPU on Linux). Harbor exposes it to the Compose network through a Caddy reverse proxy.
...
| `mlx.model` | `mlx-qwen3.5-4b` | Default model name to serve |
| `mlx.hf.path` | `mlx-community/Qwen3.5-4B-4bit` | HuggingFace repo to download |
...
Volumes work with any service name, including sub-services like `dify-api` or `ollama-init`.

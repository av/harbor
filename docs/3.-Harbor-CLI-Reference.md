...
Supported host tools are `codex`, `claude`, and `opencode`. Host tool launchers accept `--backend <service>`, `--model <model>`, and `--config`. Without an explicit backend, Harbor checks already-running OpenAI-compatible backend services in priority order and uses the first reachable `/v1/models` endpoint. Without an explicit model, Harbor uses the first advertised model from OpenAI-style `data[]` responses and common backend list variants such as Ollama-style `models[]`, then falls back to the backend's Harbor config model where available.

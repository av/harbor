...
If the first argument is not a known service handle, Harbor treats the invocation as a model pull and does not process additional arguments.
...
| `mlx.model` | `mlx-qwen3.5-4b` | Default model name for launch and integrations |
...
Creates a symlink to the `harbor.sh` script in the user's home bin directory. This allows you to run the script from any directory. Safe to re-run: existing symlinks are replaced if the target path or CLI name changed.

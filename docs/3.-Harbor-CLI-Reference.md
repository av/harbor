...
### `harbor launch [--service] <service|tool> [args]`

Launches a Harbor service CLI or supported host coding tool with the currently running Harbor services included in the compose selection. This is useful for agent and tool CLIs that should reuse an already-running Harbor inference backend instead of starting or configuring a separate backend.

```bash
# Start a backend once
harbor up ollama

# Launch mi against the active Harbor stack
harbor launch mi -p "say hello"

# Extra arguments are passed through to the service command
harbor launch mi -- -p "say hello"
...
For known Harbor service CLIs, `launch` dispatches to the same implementation as the direct command, such as `harbor mi`, `harbor promptfoo`, or `harbor plandex`. For other service handles it falls back to [`harbor run`](#harbor-run-service-command). Use `--service` before the handle to bypass host tool adapters and run a Harbor service container explicitly; this is useful for overlapping names such as `opencode`, where `harbor launch opencode` means the installed host OpenCode CLI and `harbor launch --service opencode` means the Harbor `opencode` service.
...
harbor launch codex --backend ollama --model qwen3.5:4b
harbor launch opencode --backend llamacpp --model Qwen3.5-4B
harbor launch copilot --backend ollama --model qwen3.5:4b -- -p "explain this repo"

# Claude Code uses Harbor's Ollama backend through Ollama's Anthropic-compatible API
harbor launch claude --backend ollama --model qwen3.5:4b -- -p "explain this repo"

# Print the computed launch config without starting the tool
harbor launch opencode --config
harbor launch openclaw --config

# Pass arguments to the host tool after --
harbor launch codex --model qwen3.5:4b -- --sandbox workspace-write
```

Supported host tools are aligned with Ollama's launch targets: `claude`, `codex`, `copilot`, `droid`, `hermes`, `openclaw`, `opencode`, `pi`, `pool`, and `vscode`. Host tool launchers accept `--backend <service>`, `--model <model>`, and `--config`. Without an explicit backend, Harbor checks already-running OpenAI-compatible backend services in priority order and uses the first reachable `/v1/models` endpoint. Without an explicit model, Harbor reads every advertised model from OpenAI-style `data[]` responses and common backend list variants such as Ollama-style `models[]`, then chooses a non-embedding model as the launch default when one is available.

Different tools accept backend configuration differently. Harbor uses environment variables for `claude`, `codex`, `copilot`, `hermes`, `opencode`, and `pool`; writes the documented JSON config files for `droid`, `openclaw`, and `pi`; and opens VS Code through the `code` command. Config-capable tools are given every model advertised by the selected backend, while single-model CLIs still receive one selected model for the immediate command. Use `--config` to print the selected backend/model and write any config files without starting the target tool. For colliding Harbor service names such as `opencode`, `openclaw`, and `hermes`, use `harbor launch --service <name>` to launch the container service instead of the host tool adapter.
...
Codex CLI currently uses the Responses API tool schema. With llama.cpp-family backends, Codex can fail after launch with `400 'type' of tool must be 'function'` when the backend rejects non-function tool payloads. Harbor prints this compatibility warning before invoking Codex; for live llama.cpp smoke checks, prefer `harbor launch opencode --backend llamacpp` or use Codex with a backend that accepts Codex's Responses API tool schema.

If no compatible backend is running, `launch` reports: Start a compatible backend with `harbor up ollama` or another supported backend. If `--backend <service>` names a backend that is stopped or whose OpenAI-compatible `/v1/models` endpoint is not reachable yet, `launch` exits before starting the host tool and prints the backend to start or check. Empty `/v1/models` responses require a loaded model or an explicit `--model <model>`; invalid `/v1/models` responses are treated as endpoint compatibility failures. Passing `--model <model>` skips model discovery from `/v1/models`, but Harbor still verifies that the selected backend is running and reachable.
...
The helper runs installed `codex`, `claude`, and `opencode` CLIs through `harbor launch` in non-interactive prompt mode, skips missing tools by default, and fails if a selected launched tool exits non-zero or exceeds its timeout.
...
Cross-source model management for Ollama, HuggingFace Hub, and llama.cpp caches. All subcommands operate across all three sources simultaneously — if a source is unavailable, it is skipped silently.

> Aliases: `harbor models list` = `harbor models ls`, `harbor models remove` = `harbor models rm`

#### `harbor models ls`

List all locally available models across Ollama, HuggingFace Hub, and the llama.cpp GGUF cache.
...
The command queries three sources in parallel:
...
Download a model, routing to HuggingFace or Ollama based on the model spec.
...
1. If the spec contains `/`, Harbor makes a HEAD request to `https://huggingface.co/{spec}` (5-second timeout).
2. If HuggingFace is reachable and the repo exists, the download is delegated to the `hf` service container CLI.
3. If HuggingFace is unreachable or the spec has no `/`, the spec is passed to Ollama's `pull` command.

This means a slash-containing spec falls through to Ollama gracefully when HuggingFace is offline.

#### `harbor models rm <model>`

Remove a model, searching all three sources simultaneously and deleting every match.

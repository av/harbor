...
### `harbor prismml <command>`

Runs CLI tasks specific to managing `prismml`, Harbor's PrismML llama.cpp fork server for Ternary Bonsai models.

The subcommands mirror `harbor llamacpp`: `models`, `model`, `gguf`, `args`, and `build`, plus `version` to pin the PrismML release tag. `model` accepts either a Hugging Face blob URL or a `repo:quant` spec.

```bash
harbor prismml models
harbor prismml model prism-ml/Ternary-Bonsai-2-27B-gguf:PQ2_0
harbor prismml model https://huggingface.co/prism-ml/Ternary-Bonsai-2-27B-gguf/blob/main/Ternary-Bonsai-2-27B-PTQ1_0.gguf
harbor prismml gguf /app/data/model.gguf
harbor prismml args '-fa on -c 16384'
harbor prismml version prism-b10685-7dffb15
...
Creates opt-in command names that forward to Harbor. For example, after linking `aider`, running `aider model` is equivalent to `harbor aider model`. Wrappers use the directory configured by `cli.path` (default `~/.local/bin`), which should be on your PATH. Run `harbor ln` first if needed.

```bash
harbor wrappers add aider
harbor wrappers add ollama
harbor wrappers list
harbor wrappers rm aider
harbor wrappers sync
```

`add` refuses to replace an existing file or unrelated symlink, or to shadow a command already on your PATH. Only names you add are registered in `cli.wrappers`; `rm` removes only Harbor-owned links. `sync` restores missing registered links and updates links pointing to an older Harbor installation. Harbor also syncs them when you run `harbor ln` or update Harbor.

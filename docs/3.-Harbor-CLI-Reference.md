...
Harbor binds published ports to `127.0.0.1` on a newly created bridge network by default. `harbor url --lan` only prints an address; it does not change that binding. To allow LAN access, run `harbor config set host.bind_address 0.0.0.0`, then recreate the network with `harbor down` and `harbor up` after saving any active work. To return to local-only access, set `host.bind_address` back to `127.0.0.1` and recreate the network again. Existing networks keep their previous setting until recreated. Exposing services on a LAN can make unauthenticated APIs reachable; review firewall and authentication first. Host-networked services and ports with an explicit host IP do not use this bridge-network default.

...
Docker Model Runner is Docker Desktop's local model runtime. Harbor exposes it as a backend service by starting a small proxy container while Docker manages the host-native runner. This keeps Harbor's Compose workflow intact and lets containers call an OpenAI-compatible `/v1` API.

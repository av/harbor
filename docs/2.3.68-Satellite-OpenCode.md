...
![OpenCode Screenshot](./harbor-opencode.png)
...
- Harbor discovers available backend models when no OpenCode configuration exists yet
...
Start OpenCode with `harbor-cli` when you want agents in the OpenCode container to inspect and manage Harbor services:

```bash
harbor up opencode harbor-cli
```

This mounts the Harbor home directory, Docker socket, `/harbor/harbor_cli_init.sh`, and the local LLM skill file into the OpenCode container. The image includes the Docker CLI and Compose plugin; run `sh /harbor/harbor_cli_init.sh` inside the container to place `harbor` on `PATH`.
...
# Persistent storage
HARBOR_OPENCODE_DATA               ./services/opencode/data
HARBOR_OPENCODE_CONFIG             ./services/opencode/config
...
- `HARBOR_OPENCODE_DATA` (`./services/opencode/data`) → `/root/.local/share/opencode` - Session data, logs, and auth tokens
- `HARBOR_OPENCODE_CONFIG` (`./services/opencode/config`) → `/root/.config/opencode` - Configuration files
...
- `unsloth-studio` - [Unsloth Studio](./2.3.83-Satellite-Unsloth-Studio.md) (the cross-integration mounts the bootstrapped API key so the discovered provider authenticates correctly — see [Use as an inference backend for other Harbor services](./2.3.83-Satellite-Unsloth-Studio.md#use-as-an-inference-backend-for-other-harbor-services))
- `litellm` - LiteLLM proxy (uses Harbor's configured `litellm.master.key` for the model query and generated provider)
...
Discovery runs only if `opencode.json` is absent or empty and `OPENCODE_CONFIG_CONTENT` is unset. An existing configuration is preserved across container recreations, including custom providers and model settings. To refresh an auto-generated config, first back it up by renaming `services/opencode/config/opencode.json`, then restart OpenCode with your chosen backends, for example:

```bash
mv services/opencode/config/opencode.json services/opencode/config/opencode.backup.json
harbor down opencode
harbor up --no-defaults opencode litellm
```
...
If auto-discovery is not desired, or for additional providers (Anthropic, OpenAI, etc.), configure models manually in `opencode.json` located in `HARBOR_OPENCODE_CONFIG` before starting the service. You can also set `OPENCODE_CONFIG_CONTENT` in `services/opencode/override.env`; Harbor leaves that inline configuration untouched. A nonempty manual config is not overwritten on the next `harbor up`.

For repeatable workspace setup, put an executable `init.sh` in `HARBOR_OPENCODE_CONFIG` (by default `services/opencode/config/init.sh`). Harbor runs it as the host user before model discovery or server startup on every container start. Use it to set up mounted workspaces, repositories, or tools; a failed hook is logged and OpenCode still starts. Keep credentials out of this script and mount only workspaces you trust.

For non-interactive task dispatch, mount a persistent workspace and run the CLI inside the running satellite:

```bash
docker exec harbor.opencode opencode run --model harbor-litellm/<model> --dir /root/<workspace> 'Summarize this project'
```

Use a model ID listed by `docker exec harbor.opencode opencode models harbor-litellm`. Harbor does not automatically push branches or open PRs; those actions require explicit orchestration and credentials.
...
```bash
# Edit services/opencode/override.env
...
See the [Harbor Troubleshooting Guide](./1.-Harbor-User-Guide.md#troubleshooting) for general help.
...
OpenCode starts through a Harbor entrypoint wrapper that chowns `/root` (which contains the opencode install and the data/config bind mounts) and then drops to your host user via `setpriv`, so `./services/opencode/data` and `./services/opencode/config` stay owned by your host user.

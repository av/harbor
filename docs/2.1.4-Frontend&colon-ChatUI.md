...
You can specify additional _configuration environment variables_ in the `services/chatui/configs/chatui.config.yml` file. [Official configuration docs](https://huggingface.co/docs/chat-ui/configuration/overview).
#### File ownership

The MongoDB container runs as your host user (via `HARBOR_USER_ID`); a `chatui-db-init` sidecar chowns `./services/chatui/data` before the DB starts, so database files stay owned by your host user and can be managed without `sudo`.

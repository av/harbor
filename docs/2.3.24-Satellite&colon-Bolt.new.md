...
For example, to connect `bolt` to Ollama, using the IP address on the LAN, you can use [`harbor url -a`](./3.-Harbor-CLI-Reference.md#harbor-url-service).

```bash
# For Ollama - edit services/bolt/.env.local directly
# (bolt reads .env.local at startup via its bindings.sh, not override.env)
OLLAMA_API_BASE_URL=$(harbor url -a ollama)

# For other backends (vllm in this case)
OPENAI_LIKE_API_BASE_URL=$(harbor url -a vllm)/v1
OPENAI_LIKE_API_KEY=<configured key>
```

> [!NOTE]
> Edit `services/bolt/.env.local` directly to set these values — `bolt` reads `.env.local` at startup (via its `bindings.sh`), not `override.env`. After editing, restart the service with `harbor restart bolt`. Note: the upstream production image ships without the `wrangler` CLI it needs to serve the app, so Harbor fetches a matching `wrangler@4` via `npx` on startup (requires network access on first start).
...
See [Custom Ollama Modelfiles](./2.2.1-Backend&colon-Ollama.md#custom-modelfiles) for more information on how to do this.

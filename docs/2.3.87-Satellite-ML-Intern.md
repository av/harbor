...
Auto selection is a convenience default, not a substitute for choosing the model you want. For reproducible work, list available ids and pin one explicitly:
...
When pinned, use the raw llama.cpp model id from `harbor llamacpp models`. Do not include the `llamacpp/` prefix in Harbor config; the integration adds that prefix for ML Intern.

> **Note:** ML Intern's upstream UI currently labels the configured default-model slot as "Claude Opus 4.6" even when Harbor points it at a local backend. The authoritative value is the model id returned by `curl http://localhost:34870/api/config/model`.
...
If the UI loads but agent calls fail, verify that the selected model has a reachable backend. For Ollama, start with `harbor up ml-intern ollama` and make sure `HARBOR_ML_INTERN_OLLAMA_MODEL` names a model available in Ollama. For llama.cpp, check what ML Intern selected:

```bash
curl http://localhost:34870/api/config/model
curl http://localhost:34870/api/health/llm
```

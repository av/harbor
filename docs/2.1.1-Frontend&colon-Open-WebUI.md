...
- Being default - it means most inference engines and other tools are pre-configured to connect to `webui` automatically when running together. Some examples:
  ...
  - See `$(harbor home)/services/webui/configs` for all available integrations
...
Following options must be set via [`harbor config`](./3.-Harbor-CLI-Reference#harbor-config):
...
Example of changing the host port:
```bash
harbor config set webui.host_port 80
```
...
harbor env open-webui ENABLE_REALTIME_CHAT_SAVE false

# Example: get ENABLE_REALTIME_CHAT_SAVE env variable value
harbor env open-webui ENABLE_REALTIME_CHAT_SAVE
```

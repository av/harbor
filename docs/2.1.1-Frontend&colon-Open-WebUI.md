...
> [!NOTE]
> On the very first start, Open WebUI downloads its embedding and audio (Whisper) models before the HTTP port starts serving. `harbor up` prints a notice and waits for the container to become healthy (the healthcheck allows up to 15 minutes of grace for this), and `harbor open webui` waits for a healthy status before opening the browser. Depending on your connection this can take a few minutes — the container shows as "starting" in `harbor ps` in the meantime.
...
`webui` (Open WebUI on port 8080) is a default frontend in Harbor. It is automatically included when you run bare `harbor up`. The service wires dozens of backends and tools into itself using the cross-compose mechanism: files named `compose.x.webui.<name>.yml` (and a few reverse like `compose.x.ollama.webui.yml`) mount JSON snippets into the container. On startup the entrypoint script `start_webui.sh` invokes the shared `json_config_merger.py` which renders any `${HARBOR_*}` variables, merges everything, and flattens the result into the dot-path key format (`audio.tts.engine`, `openai.api_base_urls`, ...) that current Open WebUI expects in `/app/backend/data/config.json`. Open WebUI imports that file into its per-key config store on every boot (renaming it to `old_config.json`), so the result appears instantly in the UI.
...
- `searxng` (and the `searxng.ollama` variant) — web search for RAG; enables `web.search.*` config (engine `searxng`, query URL from the injected `SEARXNG_QUERY_URL`). The `searxng.ollama` variant additionally switches RAG embeddings to Ollama (`nomic-embed-text:latest`, matching `HARBOR_OLLAMA_DEFAULT_MODELS`) at `HARBOR_OLLAMA_INTERNAL_URL`.

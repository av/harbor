...
- Being default - it means most inference engines and other tools are pre-configured to connect to `webui` automatically when running together. Some examples:
  ...
  - See `$(harbor home)/services/open-webui/configs` for all available integrations
...
open $(harbor home)/services/open-webui/configs
```
...
```bash
open $(harbor home)/services/open-webui/configs/config.override.json
```

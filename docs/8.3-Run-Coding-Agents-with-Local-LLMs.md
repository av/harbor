...
This avoids stale manual entries in `~/.grok/config.toml` taking precedence over the backend Harbor selected. `--web` and `--workflow` work the same way as with Codex or OpenCode.

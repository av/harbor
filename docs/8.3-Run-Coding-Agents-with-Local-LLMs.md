...
Supported host tools are listed in the CLI reference and include `claude`, `codex`, `copilot`, `droid`, `grok`, `hermes`, `mi`, `openclaw`, `opencode`, `pi`, `pool`, and `vscode`.
...
Harbor Launch supports backends such as [Ollama](./2.2.1-Backend&colon-Ollama.md), [llama.cpp](./2.2.2-Backend&colon-llama.cpp.md), [vLLM](./2.2.3-Backend&colon-vLLM.md), [Docker Model Runner](./2.2.22-Backend-Docker-Model-Runner.md), [MLX](./2.2.23-Backend-MLX.md), [oMLX](./2.2.24-Backend-oMLX.md), TabbyAPI, mistral.rs, SGLang, LMDeploy, Aphrodite, KTransformers, and Unsloth Studio. The exact model string depends on the backend and the model you have pulled or configured.

On Apple Silicon, host-native backends are often the best fit for coding agents that need Metal acceleration:

```bash
harbor launch --backend dmr --model ai/smollm2 codex
harbor launch --backend mlx --model mlx-community/Qwen3.5-4B-4bit opencode
...
![Boost Web Tools Integration](harbor-codex-boost.png)
*Harbor Boost provides web search and URL reading tools that can be injected into launched coding agents.*
...
Do not use `--web` with Claude Code through Harbor Launch. The launcher rejects that combination because Claude Code uses the Anthropic Messages API, while the generated Boost web workflow is exposed through OpenAI Chat Completions. Use an OpenAI-compatible host tool such as Codex, Grok, OpenCode, Copilot, Droid, OpenClaw, Pi, Pool, or Hermes when you need the `--web` path.

## Grok Build with a Local LLM

Grok Build can use Harbor's OpenAI-compatible backends through a custom model entry in `~/.grok/config.toml`. `harbor launch grok` writes or updates a `[model.harbor-<backend>]` section with the selected `base_url`, `model`, and `env_key = "XAI_API_KEY"`, then invokes `grok -m harbor-<backend>`:

```bash
harbor launch --backend ollama --model qwen3.5:4b grok -p "explain this repo"
```

The `harbor-<backend>` entry is removed from `~/.grok/config.toml` when the session ends, so it never goes stale or takes precedence over a later backend selection. `--web` and `--workflow` work the same way as with Codex or OpenCode.

## OpenCode with Local Models

![OpenCode Service](harbor-opencode.png)
*OpenCode running as a Harbor service with local backend integration.*

...
Harbor ships TGI `3.3.7` (the final release — TGI is in maintenance mode upstream) with `Qwen/Qwen3-4B` as the default model. Note that the 3.x images require a working NVIDIA driver and no longer fall back to CPU like 2.x did.

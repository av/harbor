...
HARBOR_KHOJ_VERSION                2.0.0-beta.28
HARBOR_KHOJ_DB_IMAGE               pgvector/pgvector
HARBOR_KHOJ_DB_VERSION             pg15

# Persistent storage
HARBOR_KHOJ_WORKSPACE              ./services/khoj
...
HARBOR_KHOJ_DEFAULT_MODEL          qwen3.5:4b

# Default chat model when running with llama.cpp
# (must match a model id from the llama.cpp router's /v1/models)
HARBOR_KHOJ_LLAMACPP_MODEL         LiquidAI/LFM2.5-8B-A1B-GGUF:Q8_0
```

##### Volumes

- `services/khoj/` - Workspace directory containing:
...
- `KHOJ_DEFAULT_CHAT_MODEL` set to `HARBOR_KHOJ_DEFAULT_MODEL` (default: `qwen3.5:4b`)
...
```bash
harbor up khoj llamacpp
```

This points `OPENAI_BASE_URL` at the llama.cpp router and sets `KHOJ_DEFAULT_CHAT_MODEL` from `HARBOR_KHOJ_LLAMACPP_MODEL`. Khoj auto-discovers every model the router serves (from your HuggingFace cache), so set the default to one of your cached model ids:

```bash
harbor config set khoj.llamacpp.model "LiquidAI/LFM2.5-8B-A1B-GGUF:Q8_0"
```

**Historical upstream limitation (fixed):** Khoj versions before `2.0.0-beta.28` crashed when streaming from models with `qwen3` in their name over llama.cpp (`TypeError: can only concatenate str (not "NoneType")` — Khoj's Qwen-specific thought processor didn't handle llama.cpp streaming `content: null` alongside `reasoning_content`). Harbor pins `2.0.0-beta.28+`, which includes the upstream fix ([khoj-ai/khoj#1370](https://github.com/khoj-ai/khoj/issues/1370)), so qwen3-named models now work with streaming as well.

Note: the default model only applies to fresh installs. Khoj pins each user's chat model on first run — if you change the default later, update it in the Khoj UI (settings) or the admin panel.

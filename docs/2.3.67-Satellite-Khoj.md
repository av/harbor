...
**Known upstream limitation:** avoid models with `qwen3` in their name when using llama.cpp. Khoj routes such models through a Qwen-specific streaming thought processor that crashes (`TypeError: can only concatenate str (not "NoneType")`) because llama.cpp streams `content: null` alongside `reasoning_content` for reasoning models. Non-Qwen-named models (LFM2.5, Gemma, etc.) work end-to-end, including streaming.

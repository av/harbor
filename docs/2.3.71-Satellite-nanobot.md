...
# Set a local model if you want Harbor to seed one
harbor nanobot model qwen3.5:4b
```
...
# Empty by default; set one explicitly or let Harbor inject a backend-specific value.
HARBOR_NANOBOT_MODEL
```

#### Volumes

- `HARBOR_NANOBOT_CONFIG_DIR` (`~/.nanobot`) mounted to `/home/nanobot/.nanobot` (the config dir nanobot resolves from the image's `HOME`) - Persistent configuration including API keys, channel settings, and `config.json`
...
Refer to the [Troubleshooting guide](./1.-Harbor-User-Guide.md#troubleshooting) for general Harbor issues.

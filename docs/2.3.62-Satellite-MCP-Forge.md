...
- Data stored in `services/mcpforge/data/mcp.db`
...
```bash
# Set via harbor config or in services/mcpforge/override.env
...
- `services/mcpforge/data/` - SQLite database (if using SQLite backend)
...
harbor down mcpforge
  ...
  rm -f services/mcpforge/data/mcp.db-shm services/mcpforge/data/mcp.db-wal
...
# and user has proper permissions
  ...
  chmod 755 services/mcpforge/data
```
...
```bash
  # Verify credentials
  grep ADMIN services/mcpforge/override.env

# Reset admin password
# Edit services/mcpforge/override.env and update:
...
- [MetaMCP](./2.3.42-Satellite-metamcp.md) - MCP server for managing MCP servers
- [MCPO](./2.3.43-Satellite-mcpo.md) - OpenAPI proxy for MCP tools
- [Ollama](./2.2.1-Backend&colon-Ollama.md) - Local LLM backend
- [LiteLLM](./2.3.5-Satellite&colon-LiteLLM.md) - Unified LLM proxy

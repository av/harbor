...
HARBOR_DIFY_WEBSOCKET_REPLICAS 0
```

##### Collaboration (multi-user workflow editing)

Dify's real-time collaboration feature needs a dedicated websocket server (`dify-websocket`). Harbor ships it off by default since it adds a full extra API container for a niche feature. To enable:

```bash
# Start the websocket container with the stack
harbor config set dify.websocket.replicas 1

# Turn the feature on and route /socket.io/ to the websocket server
harbor env dify ENABLE_COLLABORATION_MODE true
harbor env dify NGINX_SOCKET_IO_UPSTREAM dify-websocket:5001

harbor restart dify
```

To disable, revert the three values (`0`, `false`, `dify-api:5001`) and restart.

...
# Model name to request from the backend. Empty by default; the llamacpp
# cross-file fills it from s2s.llamacpp.model, since the llama.cpp router
# rejects requests without a model name.
harbor config get s2s.model
harbor config get s2s.llamacpp.model    # LiquidAI/LFM2.5-8B-A1B-GGUF:Q8_0
...
- The default `qwen3` TTS uses the upstream `qwentts_cpp` wheel, whose bundled ggml library links against `libcuda.so.1` - on hosts without an NVIDIA driver the container exits with `OSError: libcuda.so.1: cannot open shared object file`. The upstream image ships no alternative TTS packages (`kokoro`/`pocket` handlers exist but their modules are not installed), so s2s currently requires an NVIDIA GPU.

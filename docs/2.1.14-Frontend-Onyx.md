...
HARBOR_ONYX_FAST_MODEL             qwen3.5:4b
```
...
> **Note:** Harbor runs Onyx with Vespa as the document index (`ENABLE_OPENSEARCH_INDEXING_FOR_ONYX=false`). Recent Onyx images default to an OpenSearch sidecar that Harbor does not ship; without this flag the background worker exits waiting for OpenSearch.
...
harbor down onyx && rm -rf services/onyx/data/db && harbor up onyx
```

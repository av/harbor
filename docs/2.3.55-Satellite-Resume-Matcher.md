...
- Next.js frontend (Node.js 22) on port 34561
...
2. Pull the required Ollama models (`qwen3.5:4b` for LLM, `bge-m3` for embeddings)
...
HARBOR_RESUME_MATCHER_WORKSPACE            "./services/resume-matcher"
...
HARBOR_RESUME_MATCHER_LLM_MODEL            "qwen3.5:4b"
...
- `services/resume-matcher/data/` - Uploaded resumes, job descriptions, and SQLite database
...
```bash
# Use a different LLM
harbor config set resume-matcher.llm.model llama3.1:8b

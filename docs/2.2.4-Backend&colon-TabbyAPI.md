...
> [!NOTE]
> TabbyAPI runs exllamav2/exl3 quants and requires an NVIDIA GPU — the upstream `ghcr.io/theroyallab/tabbyapi` image is CUDA-based (no CPU or ROCm variant is published). Use `harbor up tabbyapi` together with the `nvidia`/`cdi` capability cross-files.
...
Harbor integrates with the [HuggingFaceDownloader CLI](./3.-Harbor-CLI-Reference.md#harbor-hf-dl) which can be used to download models for the TabbyAPI service.
...
# Open TabbyAPI built-in API documentation
...
- Host HuggingFace cache - `/app/models/hf`
- [llama.cpp](./2.2.2-Backend&colon-llama.cpp.md) cache - `/app/models/llama.cpp`

```bash
# Get/Set the model to run
harbor tabbyapi model turboderp/Qwen3-8B-exl3

# Get/Set extra args to pass to TabbyAPI

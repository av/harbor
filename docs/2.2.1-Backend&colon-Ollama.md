...
- **Pointing Ollama integrations at an external server**: Set `HARBOR_OLLAMA_INTERNAL_URL` (via `harbor config set ollama.internal_url http://host.docker.internal:11434` or your LAN IP) and restart consumers. Cross-compose integrations only apply when the `ollama` handle is selected; see [external Ollama without its container](#switching-to-external-ollama) for WebUI and Boost.
...
To use an external Ollama instance while keeping the Harbor `ollama` handle selected, change `HARBOR_OLLAMA_INTERNAL_URL`. Harbor's Ollama cross-compose integrations then point consumers at the external URL, but the Harbor Ollama container still starts.
...
If you omit the Harbor `ollama` handle, its cross-compose integrations do not run. In particular, setting `ollama.internal_url` alone will not give Boost an upstream model or connect WebUI directly to Ollama. Configure those two services explicitly instead:

```bash
# Ollama must accept connections from containers at this host-reachable URL.
harbor env webui OLLAMA_BASE_URL http://host.docker.internal:11434
harbor env boost HARBOR_BOOST_OPENAI_URL_EXTERNAL_OLLAMA http://host.docker.internal:11434/v1
harbor env boost HARBOR_BOOST_OPENAI_KEY_EXTERNAL_OLLAMA sk-ollama
harbor defaults rm ollama
harbor up webui boost
```

WebUI then connects to Ollama directly, while Boost discovers the same models through Ollama's OpenAI-compatible `/v1` API and advertises its module-prefixed versions in WebUI. On Linux, Harbor maps `host.docker.internal` to the Docker host gateway for both containers; native Ollama must listen on an address reachable from Docker, not only host loopback. Docker Desktop provides the host alias on macOS and Windows. Use `harbor env webui unset OLLAMA_BASE_URL` and `harbor env boost unset HARBOR_BOOST_OPENAI_URL_EXTERNAL_OLLAMA` / `HARBOR_BOOST_OPENAI_KEY_EXTERNAL_OLLAMA` to remove these overrides.
...
Harbor resolves the Ollama container image as `${HARBOR_OLLAMA_IMAGE}:${HARBOR_OLLAMA_VERSION}` (defaults to `ollama/ollama:latest`) on CPU and NVIDIA hosts. On AMD hosts, the ROCm overlay instead uses `${HARBOR_OLLAMA_IMAGE}:${HARBOR_OLLAMA_ROCM_VERSION}` (default `ollama/ollama:rocm`). Point it at a custom image when you need extra OS packages, corporate CA bundles, or a private registry mirror:
...
For an older ROCm build on AMD, set the *full* published ROCm tag. Changing `ollama.version` alone does not change the AMD image: Ollama publishes `latest` as `rocm`, but numbered releases as `<version>-rocm`, so appending `-rocm` to the standard version would break the default.

```bash
harbor config set ollama.rocm_version 0.5.7-rocm
harbor pull ollama
harbor up ollama

# Restore Harbor's moving ROCm default when finished
harbor config set ollama.rocm_version rocm
```

If you change `ollama.image`, that repository must provide the selected ROCm tag too. `ollama-init` uses a separate Alpine image and does not follow either Ollama image tag.

...
- ROCm / AMD: `compose.x.vllm.rocm.yml` mounts the two device nodes, rebuilds the service image on a ROCm base (`HARBOR_VLLM_ROCM_IMAGE`:`HARBOR_VLLM_ROCM_VERSION`, default `rocm/vllm`), and forces the explicit `python3 -m vllm.entrypoints.openai.api_server` entrypoint (required for some ROCm images).
...
When ROCm is detected on the host, Harbor automatically applies `compose.x.vllm.rocm.yml`: it mounts `/dev/kfd` and `/dev/dri`, rebuilds the service image on a ROCm base instead of the CUDA default, and starts vLLM explicitly with the OpenAI API server entrypoint (some AMD images do not provide that entrypoint themselves).

The ROCm base image is configurable separately from the CUDA one, so switching between hosts never requires touching `vllm.image`:

```bash
# Defaults (official AMD image; tags are per-GPU-architecture)
harbor config get vllm.rocm.image     # rocm/vllm
harbor config get vllm.rocm.version   # rocm7.14.0_rdna_... (vLLM 0.23)

# Pick the rocm/vllm tag matching your GPU (rdna, cdna, gfx1151, ...)
harbor config set vllm.rocm.version rocm7.14.0_cdna_ubuntu24.04_py3.14_pytorch_2.11.0_vllm_0.23.0

# Example for AMD Strix Halo / gfx1151 (verified working)
harbor config set vllm.rocm.image kyuz0/vllm-therock-gfx1151
harbor config set vllm.rocm.version stable
harbor build vllm
```
...
HARBOR_VLLM_VERSION            v0.26.0

# ROCm base image/tag, used instead of the above
# when ROCm is detected on the host
HARBOR_VLLM_ROCM_IMAGE         rocm/vllm
HARBOR_VLLM_ROCM_VERSION       rocm7.14.0_rdna_ubuntu24.04_py3.14_pytorch_2.11.0_vllm_0.23.0

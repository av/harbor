...
![Photoprism](./harbor-photoprism.png)
...
- **TensorFlow Download:** On first startup, PhotoPrism downloads TensorFlow libraries (~400-500MB). This happens once and is cached in `./services/photoprism/storage/tensorflow/` for subsequent restarts.
...
- **Storage Volume:** Thumbnails, cache, and sidecar files are stored in `./services/photoprism/storage/`. This can grow significantly depending on your library size (plan ~1-2GB per 1000 photos).
...
HARBOR_PHOTOPRISM_STORAGE                 ./services/photoprism/storage
...
- `./services/photoprism/storage` - Storage for cache, thumbnails, database sidecars, and configuration files. Also contains the `config/` folder for AI configuration.
...
The vision configuration is automatically generated from the template in `services/photoprism/vision.yml.template` using the `HARBOR_PHOTOPRISM_VISION_MODEL` setting.
...
mkdir -p ./services/photoprism/storage/config

# Create the vision configuration
cat > ./services/photoprism/storage/config/vision.yml << 'EOF'
Models:
- Type: labels
...
- Check logs: `harbor logs photoprism-db`

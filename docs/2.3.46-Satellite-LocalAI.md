...
- Harbor mounts the shared Hugging Face, llama.cpp, and vLLM caches into the container (under `/root/.cache/`); the Hugging Face cache is additionally mounted read-only at the same absolute path as on the host so GGUF symlinks in the llama.cpp cache resolve inside the container
- Unfortunately `localai` doesn't reuse cache with other services providing inference, however you can manually place the models into `$(harbor home)/services/localai/data` folder and follow ["Run Models Manually"](https://localai.io/docs/getting-started/models/#run-models-manually) guide
...
HARBOR_LOCALAI_WORKSPACE       ./services/localai/data
```

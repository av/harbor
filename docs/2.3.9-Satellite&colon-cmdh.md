...
When `cmdh` runs against one of Harbor's OpenAI-compatible backends (`llamacpp`, `dmr`, `mlx`, `omlx`, `tgi`) via the cross-service files, the model is resolved automatically from the backend's `/v1/models` list when no explicit OpenAI model name is configured.

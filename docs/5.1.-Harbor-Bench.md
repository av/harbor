...
# by default will be in the ./services/bench/defaultTasks.yml
harbor bench tasks
...
# Llamacpp config
harbor llamacpp model https://huggingface.co/lmstudio-community/Meta-Llama-3.1-8B-Instruct-GGUF/blob/main/Meta-Llama-3.1-8B-Instruct-Q8_0.gguf
harbor llamacpp args -ngl 99 --ctx-size 2048 -np 4

# Bench config
# 1. LLM
harbor bench model /root/.cache/llama.cpp/Meta-Llama-3.1-8B-Instruct-Q8_0.gguf
harbor bench api http://harbor.llamacpp:8080
harbor bench variants --temperature 0 --temperature 0.25 --temperature 0.5 --temperature 0.75 --temperature 1.0 --max_tokens 1024
# 2. Judge
harbor bench judge mistralai/mistral-nemo
harbor bench judge_api https://openrouter.ai/api
harbor bench judge_key $OPENROUTER_KEY
# 3. Common
harbor bench tasks $TASKS
harbor config set bench.parallel 4

harbor bench run --name smallama
```
...
# Download the model
harbor hf download meta-llama/Meta-Llama-3.1-8B-Instruct
```
...
# Aphrodite config
harbor aphrodite model meta-llama/Meta-Llama-3.1-8B-Instruct
harbor aphrodite args --quantization fp8 --max-model-len 2048

# Bench config
harbor bench model $(harbor aphrodite model)
harbor bench api $(harbor url -i aphrodite)
harbor bench variants --temperature 0 --temperature 1.0 --max_tokens 1024
...
# Run the bench
harbor bench run --name aphrodite-${NAME}
```

##### Dify Configuration example

Dify configuration is similar to connecting it to the other Harbor Services, you need to follow the steps from [Dify Integration](./2.3.3-Satellite&colon-Dify.md#integration) guide.
...
harbor bench model dify
harbor bench api $(harbor url -i dify)
# Reset, just in case
harbor bench variants ""
...
# Run the bench
harbor bench run --name dify-${NAME}
```
...
# Mistral.rs config
harbor mistralrs model microsoft/Phi-3.5-mini-instruct
harbor mistralrs type plain
harbor mistralrs arch phi3
harbor mistralrs isq ""
harbor mistralrs args --no-paged-attn

# Bench config
harbor bench model $(harbor mistralrs model)
harbor bench api $(harbor url -i mistralrs)
# Reset, just in case
harbor bench variants ""
...
# Run the bench
harbor bench run --name mistral-${NAME}
```
...
harbor vllm model meta-llama/Meta-Llama-3.1-8B-Instruct
harbor vllm args --quantization fp8 --max-model-len 2048

harbor bench model $(harbor vllm model)
harbor bench api $(harbor url -i vllm)
harbor bench variants ""
...
harbor bench run --name vllm-${NAME}
```
...
Example recipe to benchmark built-in [Harbor Boost](./5.2.-Harbor-Boost.md) service.
...
harbor bench api $(harbor url -i boost)

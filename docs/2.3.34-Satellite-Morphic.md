...
- Harbor runs a dedicated PostgreSQL instance (`morphic-db`) for `morphic` chat storage — data persists in `services/morphic/postgres`
- Recent `morphic` versions auto-detect available Ollama models via `OLLAMA_BASE_URL` — the model/tool_model settings below only apply to older builds
...
Additionally, all [environment variables](https://github.com/miurla/morphic/blob/main/.env.local.example) from the official example can be set according to Harbor's [environment configuration guide](./1.-Harbor-User-Guide.md#environment-variables).

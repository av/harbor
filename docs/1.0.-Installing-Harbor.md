...
Use [`install.md`](https://github.com/av/harbor/blob/main/install.md) when you want an agentic coding tool to install Harbor for you. The prompt tells the agent to orchestrate the install through Harbor's `install.sh`, choose `--install-requirements` when dependencies are missing or requested, and verify with `harbor --version` and `harbor doctor`.

...
Harbor is a containerized toolkit that runs on top of Docker, that's what enables it to support such a wide range of services and platforms. The drawback is that you'll need to have Docker installed and configured on your system.

Before proceeding to install harbor itself, you'll need:
- On Windows: [Windows Subsystem for Linux (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install)
- [git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git)
- [Docker Engine](https://docs.docker.com/engine/install/)
- [Docker Compose Plugin](https://docs.docker.com/compose/install/) ([v2.23.1](https://docs.docker.com/compose/releases/release-notes/#2231) or newer)
- If you have Nvidia GPU - [Nvidia Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html#installation)
- If you have AMD GPU - [ROCm kernel mode driver](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/how-to/docker.html#prerequisites)

Let's go through the installation steps for each of these components.

#### Windows Subsystem for Linux (WSL2)

Harbor currently relies on Bash-compatible shell for the CLI to function, on Windows it means requiring WSL2. Please follow the official guide to [install WSL2](https://learn.microsoft.com/en-us/windows/wsl/install). Harbor was tested with `ubuntu` distribution, but should work with others as well.

#### Git

Harbor currently relies on `git` for updates and version control of the CLI (even when installed via a package manager). Typically, any installation of `git` will work, it just should be available in your shell. On Windows, it should be available in WSL2.
...
Download `.msi` or `setup.exe` and run the installed.

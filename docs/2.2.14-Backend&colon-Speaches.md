### [speaches, aka faster-whisper-server](https://github.com/speaches-ai/speaches)
...
On the very first start, Harbor pre-pulls the default STT and TTS models (see `speaches.stt.model` / `speaches.tts.model`) via the `speaches-init` companion container - `harbor up` prints a notice and waits until the models are downloaded and listed by the service, so the first transcription/speech request works right away. Subsequent starts reuse the shared HuggingFace cache and are fast.

Out of the box, Harbor will:
- Share global caches (from HuggingFace, Ollama, Llama.cpp and vLLM)
  - The container runs as your host user (`HARBOR_USER_ID`/`HARBOR_GROUP_ID`), so downloads into the shared caches keep working regardless of the uid Harbor was installed under
- Pre-connect speaches as default STT and TTS backend for Open WebUI, when run together
  - ℹ️ Due to uni-directional config bindings - Harbor will override the Speech-to-Text and Text-to-Speech configuration in Open WebUI, when `speaches` is started after Open WebUI when running `speaches`, you can provide additional overrides via `open-webui/configs/config.override.json` manually if needed
...
harbor speaches stt_model Systran/faster-distil-whisper-small.en
...
harbor speaches tts_voice af_bella
...
All of the above options can also be set via the [`harbor config`](./3.-Harbor-CLI-Reference.md#harbor-config) command.
...
HARBOR_SPEACHES_STT_MODEL      Systran/faster-distil-whisper-small.en
# TTS Model in user/repo format
HARBOR_SPEACHES_TTS_MODEL      speaches-ai/Kokoro-82M-v1.0-ONNX-int8
# TTS Voice
HARBOR_SPEACHES_TTS_VOICE      af_bella
```

See Harbor's [environment configuration guide](./1.-Harbor-User-Guide.md#environment-variables) to set arbitrary environment variables for the service.

...
The following options can be set via [`harbor config`](./3.-Harbor-CLI-Reference.md#harbor-config) or in `services/homeassistant/override.env`:

```bash
# Main web UI port. Home Assistant uses host networking and always listens
# on 8123; this value only tells `harbor url`/`harbor open` where to point.
HARBOR_HOMEASSISTANT_HOST_PORT        8123
...
HARBOR_HOMEASSISTANT_WORKSPACE        ./services/homeassistant
...
Home Assistant's configuration can be customized by editing files in `$(harbor home)/services/homeassistant/config/`:
...
2. Edit `services/homeassistant/config/configuration.yaml`:

```yaml
tts:
  - platform: rest
    resource: http://localhost:33861/v1/audio/speech
```

#### Volumes

- `./services/homeassistant/config` - Configuration files, automations, scripts
- `/etc/localtime` - System timezone (read-only, mounted from host)

**Backup Considerations:**

- Back up the entire `services/homeassistant/config` directory regularly
...
- View detailed errors in: **Settings → System → Logs**
  - Restore from automatic backup in `services/homeassistant/config/.storage/backups/`

**Permission errors on Linux:**

  - Ensure the homeassistant directory is writable:
    ```bash
    chmod -R 755 services/homeassistant/config
    ```

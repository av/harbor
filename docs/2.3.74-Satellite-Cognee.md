...
# with Ollama providing Cognee's validated default models
...
- Cognee defaults to Ollama for LLM and embeddings -- start `ollama` alongside it (`harbor up cognee ollama`), or configure another backend.
...
Both the API and MCP containers run as your host user (via `HARBOR_USER_ID`); a `cognee-init` sidecar chowns the shared workspace before startup, so the sqlite/kuzu/lancedb files under `./services/cognee/workspace` stay owned by your host user.

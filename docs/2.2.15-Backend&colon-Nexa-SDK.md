...
Models are Hugging Face repos in GGUF format (`Qwen/Qwen3-0.6B-GGUF`, `NexaAI/...-GGUF`, etc.). A specific quant can be selected with a `:tag` suffix, e.g. `Qwen/Qwen3-0.6B-GGUF:Q8_0`; otherwise the CLI's default quant is used. The model store is cached on the host at `HARBOR_NEXA_CACHE` (default `~/.cache/nexa`).

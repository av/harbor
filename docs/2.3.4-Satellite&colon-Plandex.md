...
> [!NOTE]
> Under `harbor up plandex`, the CLI container stays idle (the Plandex CLI is only launched by `harbor plandex ...`) — otherwise it would exit immediately on the interactive auth prompt.

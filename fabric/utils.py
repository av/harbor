...
class Standalone:
    ...
    async def localChat(self, messages, host=''):
        ...
        response = None
        if host:
            response = await AsyncClient(host=host).chat(model=self.model,
                                                         messages=messages)
        else:
            response = await AsyncClient().chat(model=self.model,
                                                messages=messages)
    ...
    async def localStream(self, messages, host=''):
        ...
        if host:
            ...
            async for part in await AsyncClient().chat(model=self.model,
                                                       messages=messages,
                                                       stream=True):
                buffer += part['message']['content']
                print(part['message']['content'], end='', flush=True)
    ...
    async def claudeStream(self, system, user):
        ...
        async with Streamingclient.messages.stream(
                ...
                top_p=self.args.top_p) as stream:
    ...
    async def claudeChat(self, system, user, copy=False):
        ...
        if self.args.session:
            from .helper import Session
            session = Session()
            session.save_to_session(system, user, message.content[0].text,
                                    self.args.session)
    ...
    async def googleChat(self, system, user, copy=False):
        ...
        if self.args.session:
            from .helper import Session
            session = Session()
            session.save_to_session(system, user, response.text,
                                    self.args.session)
    ...
    def streamMessage(self, input_data: str, context="", host=''):
        ...
        wisdomFilePath = os.path.join(config_directory,
                                      f"patterns/{self.pattern}/system.md")
        session_message = ""
        user = ""
        if self.args.session:
            from .helper import Session
        ...
        try:
            if self.local:
                ...
                if self.args.session:
                    from .helper import Session
                    session = Session()
                    session.save_to_session(system, user, response.choices[0],
                                            self.args.session)
        except Exception as e:
            if "All connection attempts failed" in str(e):
                print(
            ...
            if "Attempted to call a sync iterator on an async stream" in str(
                    e):
                print(
        ...
        try:
            if self.client:
                ...
                if "/" in models[0] or "\\" in models[0]:
                    gptlist = [
                        item[item.rfind("/") +
                             1:] if "/" in item else item[item.rfind("\\") +
                                                          1:]
                        for item in models
                    ]
                else:
                    gptlist = [
                        item.strip() for item in models
...
class Update:
    ...
    def __init__(self):
        ...
        self.pattern_directory = os.path.join(self.config_directory,
                                              "patterns")
    ...
    def update_patterns(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            patterns_source_path = os.path.join(extracted_folder_path,
                                                "fabric-main", "patterns")
                ...
                if os.path.exists(self.pattern_directory):
                    ...
                    if custom_patterns:
                        for pattern in custom_patterns:
                            custom_path = os.path.join(self.pattern_directory,
                                                       pattern)
    ...
    def execute(self):
        with open(
                os.path.join(self.home_directory,
                             ".config/fabric/fabric-bootstrap.inc"), "w") as w:
...
class Setup:
    ...
    def update_shconfigs(self):
        bootstrap_file = os.path.join(self.config_directory,

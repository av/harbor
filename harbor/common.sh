...
function check_docker {
    if ! wsl docker --version &> /dev/null
    then
    ...
    if [[ $(wsl docker --version) =~ (([0-9]+)\.([0-9]+)([\.0-9]*)) ]]
    then
...
function check_dockercompose {
  if ! wsl docker compose version &> /dev/null && ! wsl docker-compose --version &> /dev/null
  then
  ...
  if wsl docker compose version &> /dev/null
  then
    note "$(wsl docker compose version)"
    DOCKER_COMPOSE="wsl docker compose"
  
  # or docker-compose has been installed, check its version
  elif [[ $(wsl docker-compose --version) =~ (([0-9]+)\.([0-9]+)([\.0-9]*)) ]]
  then

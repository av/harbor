...
BRIEF_CACHE_KEY = "deephop_brief_cache"


def format_skipped_status(gate_reason: str) -> str:
  """Short status line for emit_status when deephop passes through."""
...
def format_hop_query_status(hop: int, query_count: int) -> str:
  """Status line before a research hop, matching quickhop parenthesized query-count style."""
...
def is_research_heavy(text: str) -> bool:
  ...
  if RESEARCH_HEAVY_RE.search(text):
    return True
  if deliverable.has_research_signals(text) and len(text) > 20:
    return bool(
...
async def research_gate_reason(chat: "ch.Chat", llm: "llm.LLM") -> tuple[str, int]:
  ...
  if orchestrate.uses_llm_trigger(boost_config.DEEPHOP_TRIGGER.value):
...
async def plan_search_queries(
  ...
  return await orchestrate.plan_queries(
    ...
    max_queries=boost_config.DEEPHOP_MAX_QUERIES.value,
  )
...
async def synthesize_brief(
  ...
  result = await intermediate.chat_completion(
    ...
    research_summary=brief_mod.render_for_synthesis(
      brief,
      max_chars=boost_config.DEEPHOP_SYNTHESIS_MAX_CHARS.value,
    ),
...
async def run_research_loop(
  ...
  early_exit_chars = boost_config.DEEPHOP_EARLY_EXIT_CHARS.value
...
async def apply(chat: "ch.Chat", llm: "llm.LLM", config: dict | None = None):
  await workflow_mod.apply_research_module(
    ...
    cache_brief_enabled=boost_config.DEEPHOP_CACHE_BRIEF.value,

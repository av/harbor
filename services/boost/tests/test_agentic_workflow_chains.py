...
class TestAgentCodeWorkflowChain:
  ...
  @pytest.mark.asyncio
  async def test_agent_code_e2e_sightline_warn_allows_unguarded_write(self):
    """agent-code E2E: sightline warn mode allows writes but streams status."""
    chat = ch.Chat.from_conversation([
      {"role": "user", "content": AGENT_CODE_USER_MESSAGE},
    ])
    llm = _make_llm()
    ...
    try:
      ...
      with (
        ...
        await tools_module.write_file("warn-blocked.txt", "seed")
        await workflows.apply_workflow(
          deepcopy(workflow_presets.PRESETS["agent-code"]),
          chat,
          llm,
        ...
        result = await write_tool("warn-blocked.txt", "unguarded edit")
    ...
    status_messages = [call.args[0] for call in llm.emit_status.await_args_list]
    assert any("read_file required" in status for status in status_messages)

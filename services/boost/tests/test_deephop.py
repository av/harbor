"""Unit tests for the deephop Boost module."""
...
def request_context():
  ...
  try:
    ...
    if hasattr(req.state, deephop.BRIEF_CACHE_KEY):
      delattr(req.state, deephop.BRIEF_CACHE_KEY)
...
class TestDeephopBriefSynthesis:
  def test_synthesis_prompt_requires_scannable_agent_format(self):
    prompt = deephop.SYNTHESIS_PROMPT
  ...
  def test_structured_brief_schema_describes_actionable_fields(self):
    schema = deephop.StructuredBrief.model_json_schema()["properties"]
  ...
  async def test_synthesize_brief_truncates_oversized_research_summary(self):
    ...
    original = config.DEEPHOP_SYNTHESIS_MAX_CHARS.__value__
    try:
      config.DEEPHOP_SYNTHESIS_MAX_CHARS.__value__ = 8000
      ...
      with patch("research.orchestrate.cheap_llm") as cheap_llm:
        ...
        result = await deephop.synthesize_brief(
      ...
      config.DEEPHOP_SYNTHESIS_MAX_CHARS.__value__ = original


@pytest.fixture
def deephop_trigger_mode():
  original = config.DEEPHOP_TRIGGER.__value__
  yield
  config.DEEPHOP_TRIGGER.__value__ = original


@pytest.fixture
def deephop_cache_mode():
  original = config.DEEPHOP_CACHE_BRIEF.__value__
  yield
  config.DEEPHOP_CACHE_BRIEF.__value__ = original


class TestDeephopHeuristics:
  ...
  def test_skips_acknowledgments(self):
    assert deephop.research_skip_reason(self._chat("thanks!")) is not None
    assert deephop.research_skip_reason(self._chat("ok")) is not None

  def test_detects_research_heavy_migration(self):
    assert deephop.is_research_heavy(
  ...
  def test_detects_research_heavy_version_comparison(self):
    assert deephop.is_research_heavy("Compare Python 3.12 vs 3.13 asyncio API behavior")
  ...
  def test_rejects_generic_explanation(self):
    assert not deephop.is_research_heavy("Explain what asyncio.gather does in plain English.")
  ...
  async def test_needs_research_for_migration_with_module_prefix(self):
    ...
    gate_reason, _ = await deephop.research_gate_reason(chat, llm)
...
class TestDeephopLlmTrigger:
  def _chat(self, content: str) -> ch.Chat:
    return ch.Chat.from_conversation([{"role": "user", "content": content}])

  @pytest.mark.asyncio
  async def test_llm_trigger_yes_runs_classifier(self, deephop_trigger_mode):
    chat = self._chat("Compare Python 3.12 vs 3.13 asyncio API behavior")
    llm = MagicMock(module=None)
    config.DEEPHOP_TRIGGER.__value__ = "llm"
  ...
  @pytest.mark.asyncio
  async def test_llm_trigger_no_skips_research(self, deephop_trigger_mode):
  ...
  @pytest.mark.asyncio
  async def test_llm_trigger_skips_classifier_with_module_prefix(self, deephop_trigger_mode):
    chat = self._chat("Plan a migration from Django 4.2 to 5.0 for our auth layer.")
    llm = MagicMock(module=deephop.ID_PREFIX)
    config.DEEPHOP_TRIGGER.__value__ = "llm"
  ...
  @pytest.mark.asyncio
  async def test_llm_trigger_falls_back_to_heuristic_on_failure(self, deephop_trigger_mode):
    chat = self._chat("Compare Python 3.12 vs 3.13 asyncio API behavior")
    llm = MagicMock(module=None)
    config.DEEPHOP_TRIGGER.__value__ = "llm"
  ...
  @pytest.mark.asyncio
  async def test_heuristic_mode_does_not_call_classifier(self, deephop_trigger_mode):
    chat = self._chat("Compare Python 3.12 vs 3.13 asyncio API behavior")
    llm = MagicMock(module=None)
    config.DEEPHOP_TRIGGER.__value__ = "heuristic"
  ...
  @pytest.mark.asyncio
  async def test_apply_llm_trigger_skips_when_classifier_says_no(self, deephop_trigger_mode):
    chat = self._chat("Refactor the retry helper in services/boost/src/utils.py")
    llm = MagicMock(module=deephop.ID_PREFIX)
    ...
    with (
      ...
      patch.object(deephop, "plan_search_queries", new=AsyncMock()) as plan,
    ):
      cheap = MagicMock()
      cheap.chat_completion = AsyncMock(
        return_value={"needs_deep_research": False},
    ...
    llm.emit_status.assert_awaited_once_with(
      "Deephop research: skipped (llm_classifier_no)"
    )
    llm.stream_final_completion.assert_awaited_once()


class TestDeephopQueryPlanning:
  @pytest.mark.asyncio
  async def test_plan_search_queries_dedupes_and_limits(self):
    ...
    with patch("research.orchestrate.cheap_llm") as cheap_llm:
      ...
      queries = await deephop.plan_search_queries(
  ...
  async def test_plan_search_queries_uses_config_max_queries(self):
    ...
    original = config.DEEPHOP_MAX_QUERIES.__value__
    try:
      config.DEEPHOP_MAX_QUERIES.__value__ = 2
      ...
      config.DEEPHOP_MAX_QUERIES.__value__ = original


class TestDeephopGapDetection:
  ...
  async def test_detect_gaps_returns_follow_up_queries_from_llm(self):
    ...
    cheap.chat_completion.assert_awaited_once()
    kwargs = cheap.chat_completion.await_args.kwargs
    assert kwargs["schema"] is deephop.GapAnalysis
    assert kwargs["prompt"] == deephop.GAP_DETECTION_PROMPT
  ...
  async def test_detect_gaps_returns_empty_when_llm_response_not_dict(self):
    ...
    with patch("research.orchestrate.cheap_llm") as cheap_llm:
      ...
      gap = await deephop.detect_gaps(chat, llm, "Compare API v1 vs v2", brief)
  ...
  async def test_run_research_loop_triggers_hop2_for_missing_version_info(self):
    ...
    try:
      config.DEEPHOP_EARLY_EXIT_CHARS.__value__ = 15_000
...
class TestDeephopResearchLoop:
  ...
  async def test_run_research_loop_executes_two_hops(self):
    ...
    structured = deephop.StructuredBrief(
    ...
    with (
      ...
      patch.object(deephop, "detect_gaps", new=AsyncMock(return_value=gap)),
      patch.object(deephop, "synthesize_brief", new=AsyncMock(side_effect=lambda _c, _l, _m, brief: (
        brief.__class__(
          query=brief.query,
  ...
  async def test_run_research_loop_skips_second_hop_when_budget_exhausted(self):
    ...
    with (
      ...
      patch.object(deephop, "detect_gaps", new=AsyncMock(return_value=gap)),
      patch.object(deephop, "synthesize_brief", new=AsyncMock(side_effect=lambda _c, _l, _m, brief: brief)),
    ):
      brief, _ = await deephop.run_research_loop(
        chat,
        llm,
...
class TestDeephopBriefCache:
  def test_docs_note_cache_brief_is_experimental(self):
    assert "cache_brief" in deephop.DOCS
    assert "experimental" in deephop.DOCS.lower()
  ...
  @pytest.mark.asyncio
  async def test_cache_disabled_runs_research_each_time(self, deephop_cache_mode):
    config.DEEPHOP_CACHE_BRIEF.__value__ = False
    ...
    with request_context():
      ...
      with (
        patch.object(deephop, "plan_search_queries", new=AsyncMock(return_value=["stripe api migration"])) as plan,
        patch.object(deephop, "run_research_loop", new=AsyncMock(return_value=(brief, 0))) as run_loop,
      ):
        await deephop.apply(chat, llm)
        await deephop.apply(chat, llm)
  ...
  @pytest.mark.asyncio
  async def test_cache_enabled_reuses_brief_for_same_question(self, deephop_cache_mode):
    config.DEEPHOP_CACHE_BRIEF.__value__ = True
    question = "How do I migrate from Stripe API 2023-10-16 to 2024-06-20?"
    brief = ResearchBrief(query="stripe api migration")
    brief.facts = ["Checkout session fields changed in 2024-06-20"]
  ...
  @pytest.mark.asyncio
  async def test_cache_enabled_runs_research_for_different_question(self, deephop_cache_mode):
    config.DEEPHOP_CACHE_BRIEF.__value__ = True
    ...
    with request_context():
      ...
      with (
        ...
        await deephop.apply(first_chat, llm)
        await deephop.apply(second_chat, llm)
...
class TestDeephopStatusFormatting:
  def test_format_skipped_status_includes_gate_reason(self):
    assert deephop.format_skipped_status("acknowledgment") == (
      "Deephop research: skipped (acknowledgment)"
    )
    assert deephop.format_skipped_status("not_research_heavy") == (
      "Deephop research: skipped (not_research_heavy)"
    )

  def test_format_hop_query_status_matches_quickhop_style(self):
    assert deephop.format_hop_query_status(1, 1) == (
      "Deephop research: hop 1 (1 query)..."
    )
    assert deephop.format_hop_query_status(2, 3) == (
      "Deephop research: hop 2 (3 queries)..."
    )

  def test_format_hop_gathered_status_includes_hop_query_and_url_counts(self):
    assert deephop.format_hop_gathered_status(hop=1, query_count=2, pages_read=1) == (
      "Deephop research: hop 1, 2 queries, read 1 URL..."
    )
    assert deephop.format_hop_gathered_status(hop=2, query_count=1, pages_read=0) == (
      "Deephop research: hop 2, 1 query, read 0 URLs..."
    )

  def test_format_early_exit_status_includes_gathered_chars_and_threshold(self):
    assert deephop.format_early_exit_status(
      gathered_chars=15000,
      threshold=15000,
    ) == (
      "Deephop research: early exit (hop 1 gathered 15000 chars, "
      "threshold 15000), skipping hop 2..."
    )


class TestDeephopApply:
  @pytest.mark.asyncio
  async def test_apply_passes_through_on_skip(self):
    ...
    with patch.object(deephop, "plan_search_queries", new=AsyncMock()) as plan:
      await deephop.apply(chat, llm)
  ...
  async def test_apply_passes_through_when_no_queries_planned(self):
    ...
    with (
      ...
      patch.object(deephop, "run_research_loop", new=AsyncMock()) as run_loop,
    ):
      await deephop.apply(chat, llm)
    ...
    statuses = [call.args[0] for call in llm.emit_status.await_args_list]
    assert statuses == [
      "Deephop research: planning queries...",
      "Deephop research: skipped (no_queries_planned)",
    ]
    llm.stream_final_completion.assert_awaited_once()
  ...
  async def test_apply_injects_structured_brief_and_completes(self):
    ...
    with (
      patch.object(deephop, "plan_search_queries", new=AsyncMock(return_value=["stripe api migration"])),
      patch.object(deephop, "run_research_loop", new=AsyncMock(return_value=(brief, 0))),
    ):
      await deephop.apply(chat, llm)
  ...
  async def test_apply_emits_phase_statuses(self):
    ...
    with (
      ...
      patch.object(deephop, "plan_search_queries", new=AsyncMock(return_value=["k8s 1.30 deprecations"])),
    ...
    statuses = [call.args[0] for call in llm.emit_status.await_args_list]
    assert statuses[0] == "Deephop research: planning queries..."
  ...
  async def test_apply_injects_research_unavailable_note_on_total_failure(self):
    ...
    with (
      patch.object(deephop, "plan_search_queries", new=AsyncMock(return_value=["stripe api migration"])),
      patch.object(deephop, "run_research_loop", new=AsyncMock(return_value=(brief, 0))),
      patch.object(deephop.brief_mod, "has_usable_research", return_value=False),
    ):
      await deephop.apply(chat, llm)
  ...
  async def test_apply_uses_config_budget(self):
    ...
    original = (
      config.DEEPHOP_MAX_SEARCHES.__value__,
      config.DEEPHOP_MAX_URL_READS.__value__,
      config.DEEPHOP_MAX_CHARS.__value__,
    )
    try:
      config.DEEPHOP_MAX_SEARCHES.__value__ = 5
      config.DEEPHOP_MAX_URL_READS.__value__ = 4
      config.DEEPHOP_MAX_CHARS.__value__ = 15000

      with (
        patch.object(deephop, "plan_search_queries", new=AsyncMock(return_value=["openapi 3.1 changes"])),
        patch.object(deephop, "run_research_loop", new=AsyncMock(return_value=(ResearchBrief(), 0))) as run_loop,
      ):
        await deephop.apply(chat, llm)
